<script lang="ts">
    ...
    function windowCloseTransition(
        ...
        return {
            ...
            easing: sineInOut,
            css: (t) =>
                `opacity: ${t}; transform: ${existingTransform} scale(${t})`,
        }
    }
...
<section
    ...
    style="width: {+width / 16}rem;height: {+height /
        16}rem; z-index: {appZIndex}"
    tabindex="-1"
    bind:this={windowEl}
    use:draggable={{
        defaultPosition,
...
<style lang="scss">
    .container {
        ...
        box-shadow: 0px 9.9px 14.8px rgba(0, 0, 0, 0.205),
            0px 79px 118px rgba(0, 0, 0, 0.41);
        &.dark {
            & > :global(section),
            & > :global(div) {
                border-radius: inherit;
                box-shadow: inset 0 0 0 0.9px
