<script lang="ts">
    ...
    function toggleTheme() {
        if (
            wallpapersConfig[$wallpaper.id].type === 'dynamic' &&
            $wallpaper.canControlTheme
        ) {
...
>
    ...
    >
        <ActionCenterTile grid={[1, 1]}>
            <button
                class="toggle"
                class:filled={$theme.scheme === 'dark'}
                    ...
                    {#each Object.keys(colors) as colorID}
                        <button
                            style="--color-hsl: {colors[colorID][$theme.scheme]
                                .hsl}; --color-contrast-hsl: {colors[colorID][
                                $theme.scheme
                            ].contrastHsl}"
    ...
    >
        <ActionCenterTile
            ...
            on:keyup={(e) =>
                ['Enter', 'Space Bar'].includes(e.key) && openWallpapersApp()}
        >
            <div class="wallpaper-tile">
                <img
                    class="wallpaper-thumbnail"
                    src="/wallpapers/{wallpapersConfig[$wallpaper.id]
                        .thumbnail}.jpg"
...
<style lang="scss">
    .container {
        ...
        box-shadow: hsla(0, 0%, 0%, 0.3) 0px 0px 11px 0px,
            inset 0 0 0 var(--border-size)
                hsla(var(--system-color-dark-hsl), 0.3),
